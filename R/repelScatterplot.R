                                ...
                                transform_coord = "npc")
{
  ...
  if(!is.null(label.sel))
    boxdf <- boxdf[label.sel,]
  ...
  if(!is.null(label.sel))
  {
    posdf <- posdf[label.sel,]
    box_x <- box_x[label.sel]
    box_y <- box_y[label.sel]
  }
                                ...
                                ...)
{
  ...
  coords <- vlfunctions::get_repelled_labels(x = x,
                                             y = y,
                                             labels = labels,
                                             label.sel = label.sel,
                                             cex = label.cex,
  ...
  if(!is.null(label.sel))
  {
    rect.col <- rect.col[label.sel]
    label.col <- label.col[label.sel]
  }
  ...
  coords[, seg_length:= seg_length.x>strwidth("M", cex= label.cex/10) | seg_length.y>strheight("M", cex= label.cex/5)]

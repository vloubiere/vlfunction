                             ...
                             nTrim5= 0)
{
  ...
  stats <- capture.output(align(index = Rsubread_index_prefix,
                                readfile1 = fq1, 
                                readfile2 = fq2, 
                                type= "dna", 
                                output_file = output, 
  ...
  cmd <- paste("module load build-env/2020; module load samtools/1.9-foss-2018b; samtools view -@", 
               ...
               output)
  # Chromosome infos
  chr <- fread(list.files(dirname(Rsubread_index_prefix), ".reads$", full.names = T), 
               col.names = c("length", "seqnames"))
  # Import reads
  reads <- fread(cmd= cmd, fill= T, nrows = 1e6)
  # Clean
  ...
  cov <- coverage(reads)/total_reads*1e6

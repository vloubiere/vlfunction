...
#' @param grid_hex Height of grid lines
#' @param cex.inter Cex size intersection labels
#' @param show_empty Should empty instersections be shown? default= F
                          ...
                          las= par("las"))
{
  ...
  dat[, .id:= factor(.id, names(dat_list))]
  setnames(dat, "V1", "var")
  # Intersections ----
  inter <- dcast(dat, 
                 var~.id, 
  ...
  grid[, y:= par("usr")[3]-strheight("M", cex= grid_hex*1.25)]
  grid[set, y:= y-strheight("M", cex= grid_hex*1.25)*i.I, on= "variable==.id"]
  ...
  set[, top:= y+strheight("M", cex= grid_hex)/2]
  set[, height:= strheight("M", cex= grid_hex)]

...
#'                      submit= FALSE)
#' 
#' # Once the jobs are finished, you can check the size of output files:
...
#' @param FC_metadata_output An .rds path where to save the metadata file, which contains the directories containing .dds (DESeq2 objects) and FC table files and will be used to manage them. By default, when the processed_metadata is a path to a processed_metadata files, "_FC_tables.rds" will be appended to the processed_metadata file path. 
#' @param Rpath Path to an Rscript executable. Default= "/software/f2022/software/r/4.3.0-foss-2022b/bin/Rscript".
...
#' processed <- readRDS("Rdata/metadata_PROseq_processed.rds")
#' 
#' # Modify the experiment column of the processed metadata if needed and save it as a txt file
#' # new_processed <- data.table::copy(processed)
#' # new_processed$experiment <- "all"
#' # fwrite(new_processed, "Rdata/new_processed.txt")
#' 
#' # Differential analysis
#' vl_PROseq_DESeq2(processed, # Or new processed "Rdata/new_processed.txt"
  ...
  meta <- if(grepl(".rds$", processed_metadata))
    readRDS(processed_metadata) else if(grepl(".txt$", processed_metadata))
      fread(processed_metadata) else
        stop("processed_metadata should either be in .rds or .txt format")
